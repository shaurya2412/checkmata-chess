...
const MovesTable: React.FC<MovesTableProps> = ({ onResign, onDraw }) => {
  ...
  return (
    <div className="text-[#C3C3C0] relative w-full ">
      ...
      {moves.length > 0 || onResign || onDraw ? (
        <div className="w-full p-2 bg-[#20211D] flex items-center justify-between">
          <div className="flex gap-4">
            {onDraw && (
              <button
                onClick={onDraw}
                className="flex items-center gap-2 hover:bg-[#32302E] disabled:opacity-50 disabled:cursor-not-allowed rounded px-2.5 py-1"
              >
            ...
            {onResign && (
              <button
                onClick={() => {
                  console.log('Resign button clicked', { onResign: !!onResign });
                  if (onResign) {
                    onResign();
                  } else {
                    console.warn('onResign handler is not provided');
                  }
                }}
                className="flex items-center gap-2 hover:bg-[#32302E] disabled:opacity-50 disabled:cursor-not-allowed rounded px-2.5 py-1"
              >
