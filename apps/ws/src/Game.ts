...
export class Game {
  ...
  async resign(user: User) {
    ...
    // player1UserId is white, player2UserId is black
    // If the resigning user is player2 (black), white wins
    // If the resigning user is player1 (white), black wins
    console.log('Game.resign called', {
      userId: user.userId,
    ...
    if (!this.player2UserId) {
      console.error('Cannot resign: game not fully started (no player2)');
      return;
    }

    const result = user.userId === this.player2UserId ? 'WHITE_WINS' : 'BLACK_WINS';
    console.log('Resigning with result:', result);
    this.endGame('COMPLETED', result);
  }
