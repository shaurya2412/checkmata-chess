  ...
  private addHandler(user: User) {
    user.socket.on('message', async (data) => {
      ...
      if (message.type === RESIGN) {
        const gameId = message.payload?.gameId;
        console.log('RESIGN message received', { gameId, userId: user.userId, payload: message.payload });

        if (!gameId) {
          console.error('RESIGN: No gameId in payload', { payload: message.payload });
          return;
        }
        ...
        if (game) {
          console.log('Resigning game', { gameId, gameFound: true });
          try {
            await game.resign(user);
            this.removeGame(game.gameId);
          } catch (error) {
            console.error('Error during resign:', error);
          }
        } else {
          console.warn('RESIGN: Game not found', {
            gameId,
            availableGames: this.games.map((g) => ({ id: g.gameId, p1: g.player1UserId, p2: g.player2UserId })),
          });
        }
      }
